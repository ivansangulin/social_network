...
export const PopoverContent = ({ open }: { open: boolean }) => {
  ...
  useEffect(() => {
    const handleNewNotification = ({
      ...
      setNotifications((notifications) => {
        return [{ ...notification, read: open }, ...notifications];
      });
      if (open) {
      ...
      <PopoverPanel className="flex flex-col p-2 rounded-e-3xl absolute top-0 left-full min-w-[28rem] h-svh bg-white border-secondary border-l">
        {({ close }) => (
          <div className="flex flex-col px-2">
            ...
            <hr />
            <div className="flex flex-col mt-2 space-y-2 overflow-y-auto scrollbar-thin">
              {notifications.length > 0 ? (
                notifications.map((notification) => (
                  <div
                    className={`py-4 px-2 flex space-x-2 justify-between items-center rounded-2xl ${
